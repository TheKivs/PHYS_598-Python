...
\section{PHYS 598 SDA RECITATION 8 - PROBLEM SET}


\textbf{1. The Moore-Penrose Pseudoinverse:} Given an $m\times n$ matrix $\sB$, the Moore-Penrose (generalized) matrix inverse is a \emph{unique} $n\times m$ matrix pseudoinverse $\sB^+$. This matrix was independently defined by Moore in 1920 and Penrose (1955), and variously known as the generalized inverse, pseudoinverse, or Moore-Penrose inverse. 
\vspace{1 mm}

Browse through the theorems and examples in \href{http://www.math.ucla.edu/~laub/33a.2.12s/mppseudoinverse.pdf}{\texttt{this Math@UCLA link}}. Your task then is to solve the exercises at end of the PDF. Please verify with me your proof of either part (c) or (d).\\

\textbf{2. The Khatri-Rao product:}  Let $\sA$ and $\sB$ be $I\times K$ and $J \times K$ matrices, respectively. The \href{https://en.wikipedia.org/wiki/Kronecker_product#Khatri-Rao_product}{Khatri-Rao product} $\sA\odot \sB$ satisfies the following properties
\begin{itemize}
\item
$(\sA\odot \sB)^t(\sA\odot \sB) = (\sA^t\sA)* (\sB^t\sB)$, where $*$ denotes the \href{https://en.wikipedia.org/wiki/Hadamard_product_(matrices)}{Hadamard product}, and
\item $(\sA\odot \sB)^+ = ((\sA^t\sA)*(\sB^t\sB))^+(\sA\odot \sB)^t$, where $^+$ denotes the Moore-Penrose pseudoinverse.
\end{itemize}

\textbf{(a)} The proofs of the above properties are worked out in the lecture notes (or just google!). Your task is to work out explicitly the first property for generic matrices with $I=J=2$ and $K=3$, preferably on a computer. (\emph{Prof. Song used a different notation for these products than what is more commonly used. We shall stick to the notation used in class, i.e. the ones above.})
\vspace{2 mm}


\textbf{(b)} Complete the proof of theorem \textbf{A.6.} in the lecture notes. For your convenience, the statement is
\[(\sA \otimes \sB)^t = (\sA^t\otimes \sB^t).\]

\textbf{(c)} Let $\sA_1,\ldots,\sA_p$ be orthogonal matrices. Then prove that $\sA_1\otimes \ldots \otimes \sA_p$ is an orthogonal matrix.
\end{document}
